 ...
 * the players named as the leaderboard names them; that the game opens on
 * the position as it stands — the moves already played are put on the board
 * at once and in silence, never replayed, and no row of recent moves is
 * shown under it; that moves and clocks then follow the snapshots without a
 * reload; that nothing a player could do is possible; that the result is
 * shown when the server says so, with the way home and, once there is one,
 * the next game; that leaving puts the board back; and that /spectate/<id>
 * boots straight into the same game.
 *
...
/* Sounds the page played while `f` ran: soundFor() looks at G.sound before
   it plays anything, so the flag being on throughout says every move was
   heard, and off says none was. The stub has no AudioContext worth reading,
   so the flag is what is watched. */
const soundsOn = p => p.G.sound === true;
...
async function main(){
  ...
  check('the game opens on the position as it stands: both moves on the board at once', p.G.uci.join(' ') === 'e2e4 e7e5' && p.G.sans.join(' ') === 'e4 e5', { uci: p.G.uci, sans: p.G.sans });
  check('...the pawns where the game has them', p.G.st.b[p.sqIndex('e4')] && p.G.st.b[p.sqIndex('e4')].t === 'P' && !p.G.st.b[p.sqIndex('e2')] && p.G.st.b[p.sqIndex('e5')] && !p.G.st.b[p.sqIndex('e7')]);
  check('...nothing waiting to be replayed, and no replay to wait for', !('queue' in p.SPEC) && !('timer' in p.SPEC) && !/SPEC_REPLAY|specStep|specQueue|specHalt/.test(SRC));
  check('...the sound left as it was', soundsOn(p));
  ...
  check('there is no row of recent moves under the board', !IDS.includes('specMoves') && !/specMoves|SPEC_RECENT/.test(SRC));
  check('...nor a height the board pays for one', !/body\.spectating \.game-layout\{--board-max/.test(SRC));
  check('...and the foot says nothing of a replay', hint(p) === 'SPECTATING · WHITE TO MOVE' && !/FROM THE START/.test(SRC), hint(p));
  ...
  check('a move arrives without a reload, and is played at once — the game is live', p.G.uci.length === 3 && p.G.sans[2] === 'Nf3');
  ...
  check('two moves at once are both on the board at once, in order', p.G.uci.join(' ') === g3.moves && p.G.sans.join(' ') === 'e4 e5 Nf3 Nc6 Bc4', p.G.uci);
  check('...the foot saying whose move it is', hint(p) === 'SPECTATING · BLACK TO MOVE', hint(p));
  check('...and the clocks running', p.clockRunning() === true);
  // the stub's appendChild keeps every element ever added, so a count that
  // grows by whole sets of men is a board drawn from fresh elements rather
  // than one whose pieces were slid into place: one set while the moves were
  // put on (the rebuild emptied the board), and one for the final drawing
  const drawn = p.doc.getElementById('piecesLayer').children.length;
  sock.push(watchMsg(g2));
  check('a snapshot that is not a continuation rebuilds the position, and shows it at once', p.G.uci.join(' ') === g2.moves && p.G.sans.join(' ') === 'e4 e5 Nf3', { uci: p.G.uci });
  check('...the board drawn fresh: every man a new element, none slid from where it stood', p.doc.getElementById('piecesLayer').children.length - drawn === 64, p.doc.getElementById('piecesLayer').children.length - drawn);
  ...
  check('the next game opens on its position too', p.G.uci.join(' ') === 'd2d4', p.G.uci);
  ...
  check('...opening on the position as it stands, from the chair of whoever is to move (black)', pf.G.uci.join(' ') === 'd2d4 d7d5 c2c4' && pf.G.human === 'b' && pf.G.st.turn === 'b', { uci: pf.G.uci, human: pf.G.human, turn: pf.G.st.turn });
  fs1.push(watchMsg(snap('game-fog-1', 'fog', 'd2d4 d7d5 c2c4 e7e6', { sans:['d4','d5','c4','e6'] })));
  check('the next move is played as it happens', pf.G.uci.join(' ') === 'd2d4 d7d5 c2c4 e7e6' && pf.clockRunning() === true, pf.G.uci);
  ...
  check('...and a first move is played on it', pb.G.uci.join(' ') === 'e2e4');
  ...
  check('a link to a finished game shows how it ended, at once', pr.up('endOverlay') && pr.text('endTitle') === 'Draw' && pr.text('endText') === 'Drawn by agreement.', pr.text('endTitle'));
  ...
  check('(the position is on the board)', pp.G.uci.length === 4);
  pp.loc.pathname = '/';
  for (const f of pp.win.popstate) f();
  check('the browser\'s back leaves the game', pp.screen() === 'home' && !pp.SPEC.on);
  await wait(120);
  check('...and nothing of it is left on the board', pp.G.uci.length === 0);
