...
def main():
    ...
    check("no bot was left standing in the queue",
          any(c.is_ai for q in server.lobby.values() for c in q), False)
    ...
    print("\n\033[1mPlaying it again\033[0m")

    # Rematch asks the opponent, and there is nobody there to ask: an
    # invitation addressed to a BotClient would sit until it lapsed. So a
    # rematch of a bot game is granted instead — the same opponent, the same
    # terms, colours swapped, and no invitation anywhere in it.
    clear()
    server.AI_WAIT = 0.15
    encore = FakeClient("E1", user_id="acct-e1")
    key = ("blind", 5, 0, "ranked")
    with server.lock:
        server.lobby[key] = [encore]
        encore.queue_key = key
        server.arm_ai_fallback(encore, key)
    time.sleep(0.6)
    first = encore.got("start")[0]
    server.handle_resign(encore)
    encore.sent = []
    server.handle_rematch(encore, {})
    starts = encore.got("start")
    check("a rematch of a bot game starts one straight away", len(starts), 1)
    check("nobody was invited", encore.got("rematch-sent"), [])
    check("and nothing is left waiting to be answered", server.rematches, {})
    second = starts[0] if starts else {}
    check("it is the same opponent", second.get("opponent"), first["opponent"])
    check("at the rating it was playing at",
          second.get("ai", {}).get("elo"), first["ai"]["elo"])
    check("still declared a bot", second.get("ai", {}).get("bot"), True)
    check("the terms are the ones just played",
          (second.get("mode"), second.get("minutes"), second.get("inc"), second.get("kind")),
          ("blind", 5, 0, "ranked"))
    check("and the colours swap", second.get("color") != first["color"], True)
    check("it is a new game, not the finished one",
          second.get("game") != first["game"], True)
    server.drop_client(encore)
