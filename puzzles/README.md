...
Anything asked for as "make puzzles", "generate puzzles", "add puzzles", or any
new feature containing puzzles, is generated by **this** pipeline —
`tools/generate_puzzles.js` then `tools/verify_puzzles.js`, both at their
defaults — unless the request explicitly names a different standard. The
defaults are the standard; passing a shallower depth, a wider tolerance or
`--no-payoff` to make a run cheaper or a category fuller is the thing the rule
exists to forbid. The full list of gates, and why each one is there, is in
`CLAUDE.md` under "Making puzzles: the standing rule".

Three things that are easy to get wrong and are settled:

**A new corpus excludes the old ones, by default.** Any run that supplements an
existing corpus, or that is meant to hold genuinely new puzzles, passes
`--excludeIn` naming every corpus it must not overlap — the phase tracks here,
`modes/` including `reserve.json`, `daily/`, and `practices/`. It is not
optional and does not wait to be asked for. The generator seeds its `seen` set
with those fens, so a position already shipped is skipped before an engine
touches it, and `tools/daily_assign.js` re-asks at the deal.

Match on the **position**, never only on the id. `puzzleId()` is
`sha1(fen + '|' + moves)`, so a duplicate id already implies a duplicate fen —
excluding on the fen catches every duplicate id *and* the case id-matching
misses: one position found twice and extended differently is two ids and one
position. Reuse is allowed only when it has been explicitly asked for; a re-cut
of an existing pool (`--poolsIn`) is the normal case for that.

**Phase is metadata.** `opening`, `middlegame` and `endgame` are a label
`bucketFor()` stamps on a record *after* it has cleared the whole standard, plus
three files so each has its own resumable progress log. They are never a
separate standard and never a quota. With a flat `--games`, `need` is 0 and
every bucket is unconditionally wanted.

**The Practices are a different system.** `tools/practice_rules.js` holds their
standard and is reached only through `tools/generate_practices.js`.
`verify_puzzles.js` does not require it and must not. The Middle Game Practice
`+35` rule (`MG_SWING_MIN`) never applies to a puzzle because its position
happens to be a middlegame.

**Quality outranks the count.** A track is however many positions cleared the
standard — which is why the three files here are 58, 465 and 265 rather than a
round hundred each. If a hundred are asked for and sixty survive, sixty ship and
the shortfall is reported. Padding a ladder to a round number means keeping
puzzles that did not earn their place, and one bad puzzle costs more than ten
missing ones.

Infrastructure may improve — speed, checkpointing, parallelism, storage — and
must do so without making the chess cheaper. `--excludeIn` and `--gameBudget`
are the pattern: default off, and checked *between* candidates so nothing is
ever judged at reduced depth.
