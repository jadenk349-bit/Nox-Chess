...
Eleven records still lack a position from a real game, and the reason is now
measured rather than guessed. **Ten of the eleven have no matcher at all** —
...
The eleventh, `smothered-mate`, HAS a matcher, and was measured directly: zero
...
every replayed game, which is how **seven of the original eighteen** were filled
this pass: `clearance`, `greek-gift`, `knight-fork`, `desperado`, `interference`, `x-ray` and
`zwischenzug` — the last of those a concept whose own detectability is `human-only`,
which is the strongest evidence that this road does not need a detector. That road works, it is open, and each concept costs one test.
