...
| 2 | Important concepts have strong recognition criteria | **PARTIAL** | 53 of the 83 concepts whose own record permits a detector have one, up from 32. But *having* a matcher is not the standard: `state/TRAPS.md` measures every matcher against the false-positive traps its record states, and **25 of 108 traps have never been written about either way**. Ten defects have been found by reading that list; the rest of it is the honest size of what is unchecked. |
...
master games · 661 tests + 407 API + 374 audit + 3574 explanation assertions.
...
7. **25 of 108 stated traps have never been read against their matcher.**
   `state/TRAPS.md` is the list. Ten defects have come out of the 83 that have
   been read — including one in Layer 3 that had been true of every measurement
   in the project — at a rate that has not yet fallen, so the honest expectation
   is that more are in the remaining 25.
