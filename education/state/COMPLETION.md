...
Eighteen records lacked a position from a real game when the pass began. **Six
...
**A detector was never what the evidence needed.** Twelve of the eighteen were
...
- Three by MATERIAL — `knight-endgames`, `rook-endgame-principles` and
  `mixed-piece-endgames` are categories defined by what is on the board, which
...
Six remain: `double-attack`, `fortress`, `lpdo`, `piece-properties`,
`queen-endgames`, `smothered-mate`. `smothered-mate` is measured rather than
outstanding — zero occurrences in 2,822,256 positions, because a master resigns
into one rather than allowing it to be played. `queen-endgames` wants queens and
pawns only, which the same scan found none of in 330 games. The other four each
cost one test in the miner, and the road is open.
