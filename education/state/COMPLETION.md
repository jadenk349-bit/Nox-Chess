...
Twelve records still lack a position from a real game, and the reason is now
measured rather than guessed. **Eleven of the twelve have no matcher at all** —
...
The twelfth, `smothered-mate`, HAS a matcher, and was measured directly: zero
...
every replayed game, which is how **six of the original eighteen** were filled
this pass: `clearance`, `greek-gift`, `knight-fork`, `desperado`, `interference` and
`x-ray`. That road works, it is open, and each concept costs one test.
