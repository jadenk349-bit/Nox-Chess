...
Eighteen records lacked a position from a real game when the pass began. **Five
...
**A detector was never what the evidence needed.** Thirteen of the eighteen were
filled by asking the record's own sentence directly:

- Seven by geometry, in `tools/mine_master_motifs.js` — `clearance`,
  `greek-gift`, `knight-fork`, `desperado`, `interference`, `x-ray` and
  `zwischenzug`. The last of those has `human-only` detectability and its own
...
Five remain: `double-attack`, `fortress`, `lpdo`, `piece-properties` and
...
asking the material directly found 372. The other four each cost one test in the
