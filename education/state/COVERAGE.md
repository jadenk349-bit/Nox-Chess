# Concept coverage report

GENERATED by `tools/depth_report.py --markdown`. Do not edit by hand.

`x` has it · `-` missing · `.` not applicable to this concept

Columns: **def** definition, **term** terminology, **src** provenance, **attr** attribution, **recog** recognition, **exc** exceptions, **pos** positive example, **neg** negative example, **amb** ambiguous example, **eng** engine validation, **tb** tablebase validation, **game** master game, **fp** false positive test, **beg** beginner explanation, **adv** advanced explanation, **tmpl** explanation template

Mean completeness **0.93** across 137 concepts.

- **full**: 6 areas
- **substantial**: 10 areas
- **partial**: 69 areas

69 of 85 areas contain exactly ONE concept. The rating rule needs two concepts for *substantial* and three for *full*, so those areas cannot rise however complete the record is: `two-weaknesses` scores 1.00 and is rated *partial*, the same word as an area scoring 0.45. Rating a one-concept area by its one concept, at a **higher** bar (0.85 for *full* rather than 0.75 for a mean of three), gives: **full** 61, **substantial** 14, **partial** 10.

The completion assessment is judged on the ORIGINAL rule and criterion 1 stays where that rule puts it. This second reading is printed so the artefact is visible, not so it can be spent.


## Areas

| rating | mean | area | concepts |
|---|---|---|---|
| full | 1.00 | `pawn-breaks` | 3 |
| full | 0.99 | `pawn-structures` | 5 |
| full | 0.94 | `named-theoretical-positions` | 3 |
| full | 0.93 | `tactical-motifs` | 19 |
| full | 0.92 | `official-rules` | 12 |
| full | 0.89 | `opening-theory-concepts` | 4 |
| substantial | 1.00 | `checks-captures-threats` | 2 |
| substantial | 1.00 | `restriction` | 2 |
| substantial | 1.00 | `king-safety` | 2 |
| substantial | 1.00 | `positional-sacrifices` | 2 |
| substantial | 1.00 | `gambit-principles` | 2 |
| substantial | 1.00 | `bishop-endgames` | 2 |
| substantial | 0.97 | `named-mating-patterns` | 2 |
| substantial | 0.97 | `weak-squares` | 2 |
| substantial | 0.93 | `attack-principles` | 2 |
| substantial | 0.65 | `blindfold-visualisation` | 4 |
| partial | 1.00 | `notation` | 1 |
| partial | 1.00 | `board-geometry` | 1 |
| partial | 1.00 | `ranks-files-diagonals` | 1 |
| partial | 1.00 | `tactical-combinations` | 1 |
| partial | 1.00 | `forcing-moves` | 1 |
| partial | 1.00 | `positional-evaluation` | 1 |
| partial | 1.00 | `initiative` | 1 |
| partial | 1.00 | `defensive-principles` | 1 |
| partial | 1.00 | `prophylaxis` | 1 |
| partial | 1.00 | `overprotection` | 1 |
| partial | 1.00 | `piece-coordination` | 1 |
| partial | 1.00 | `good-bad-bishops` | 1 |
| partial | 1.00 | `bishop-pair` | 1 |
| partial | 1.00 | `knights` | 1 |
| partial | 1.00 | `piece-exchanges` | 1 |
| partial | 1.00 | `worst-placed-piece` | 1 |
| partial | 1.00 | `development` | 1 |
| partial | 1.00 | `tempo` | 1 |
| partial | 1.00 | `space` | 1 |
| partial | 1.00 | `center-control` | 1 |
| partial | 1.00 | `outposts` | 1 |
| partial | 1.00 | `open-files` | 1 |
| partial | 1.00 | `semi-open-files` | 1 |
| partial | 1.00 | `exchange-sacrifices` | 1 |
| partial | 1.00 | `material-imbalances` | 1 |
| partial | 1.00 | `static-vs-dynamic` | 1 |
| partial | 1.00 | `two-weaknesses` | 1 |
| partial | 1.00 | `transformation-of-advantages` | 1 |
| partial | 1.00 | `opening-principles` | 1 |
| partial | 1.00 | `middlegame-principles` | 1 |
| partial | 1.00 | `endgame-principles` | 1 |
| partial | 1.00 | `king-activation` | 1 |
| partial | 1.00 | `opposition` | 1 |
| partial | 1.00 | `zugzwang` | 1 |
| partial | 1.00 | `pawn-endgames` | 1 |
| partial | 1.00 | `drawing-mechanisms` | 1 |
| partial | 1.00 | `conversion-techniques` | 1 |
| partial | 1.00 | `historical-principles` | 1 |
| partial | 1.00 | `classical-school` | 1 |
| partial | 1.00 | `hypermodern-school` | 1 |
| partial | 1.00 | `soviet-school` | 1 |
| partial | 1.00 | `modern-dynamic` | 1 |
| partial | 0.94 | `piece-activity` | 1 |
| partial | 0.94 | `rooks` | 1 |
| partial | 0.94 | `rook-endgames` | 1 |
| partial | 0.94 | `knight-endgames` | 1 |
| partial | 0.94 | `queen-endgames` | 1 |
| partial | 0.94 | `mixed-piece-endgames` | 1 |
| partial | 0.94 | `fortresses` | 1 |
| partial | 0.93 | `named-tactical-patterns` | 1 |
| partial | 0.93 | `strong-squares` | 1 |
| partial | 0.93 | `rules-of-thumb` | 1 |
| partial | 0.93 | `queen` | 1 |
| partial | 0.92 | `triangulation` | 1 |
| partial | 0.88 | `piece-properties` | 1 |
| partial | 0.85 | `exceptions-to-rules` | 1 |
| partial | 0.80 | `candidate-moves` | 1 |
| partial | 0.77 | `corresponding-squares` | 1 |
| partial | 0.73 | `planning` | 1 |
| partial | 0.67 | `chess-terminology` | 1 |
| partial | 0.67 | `named-strategic-concepts` | 1 |
| partial | 0.67 | `beginner-misconceptions` | 1 |
| partial | 0.60 | `calculation-principles` | 1 |
| partial | 0.60 | `practical-principles` | 1 |
| partial | 0.60 | `time-management` | 1 |
| partial | 0.60 | `practical-psychology` | 1 |
| partial | 0.58 | `intermediate-concepts` | 1 |
| partial | 0.58 | `advanced-concepts` | 1 |
| partial | 0.58 | `master-concepts` | 1 |

## Concepts

| concept | score | def | term | src | attr | recog | exc | pos | neg | amb | eng | tb | game | fp | beg | adv | tmpl |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| `board-geometry` | 1.00 | x | x | x | . | x | . | x | x | . | x | x | . | x | x | x | x |
| `ranks-files-diagonals` | 1.00 | x | x | x | . | x | x | x | x | . | x | . | . | x | x | x | x |
| `blockade` | 1.00 | x | x | x | x | x | x | x | x | x | x | x | x | x | x | x | x |
| `defensive-principles` | 1.00 | x | x | x | x | x | x | x | x | x | x | . | x | x | x | x | x |
| `overprotection` | 1.00 | x | x | x | x | x | x | x | x | x | x | . | x | x | x | x | x |
| `prophylaxis` | 1.00 | x | x | x | x | x | x | x | x | x | x | . | x | x | x | x | x |
| `restraint` | 1.00 | x | x | x | x | x | x | x | x | x | x | . | x | x | x | x | x |
| `center-control` | 1.00 | x | x | x | x | x | x | x | x | x | x | . | x | x | x | x | x |
| `initiative` | 1.00 | x | x | x | x | x | x | x | x | x | x | . | x | x | x | x | x |
| `opening-development` | 1.00 | x | x | x | x | x | x | x | x | x | x | . | x | x | x | x | x |
| `space` | 1.00 | x | x | x | x | x | x | x | x | x | x | . | x | x | x | x | x |
| `tempo` | 1.00 | x | x | x | . | x | x | . | . | . | . | . | . | x | x | x | x |
| `endgame-principles` | 1.00 | x | x | x | x | x | x | x | x | x | x | x | x | x | x | x | x |
| `key-square` | 1.00 | x | x | x | . | x | . | x | x | . | x | x | . | x | x | x | x |
| `king-activation` | 1.00 | x | x | x | x | x | x | x | x | x | x | x | x | x | x | x | x |
| `opposite-coloured-bishops` | 1.00 | x | x | x | . | x | x | x | x | x | x | x | x | x | x | x | x |
| `opposition` | 1.00 | x | x | x | x | x | . | x | x | . | x | x | . | x | x | x | x |
| `perpetual-check` | 1.00 | x | x | x | . | x | x | x | x | . | x | . | . | x | x | x | x |
| `wrong-rook-pawn` | 1.00 | x | x | x | x | x | . | x | x | . | x | x | . | x | x | x | x |
| `zugzwang` | 1.00 | x | x | x | x | x | . | x | x | . | x | x | . | x | x | x | x |
| `classical-school` | 1.00 | x | x | x | x | x | x | x | x | x | x | . | x | x | x | x | x |
| `historical-principles` | 1.00 | x | x | x | x | x | x | x | x | x | x | x | x | x | x | x | x |
| `hypermodern-school` | 1.00 | x | x | x | x | x | x | x | x | x | x | . | x | x | x | x | x |
| `modern-dynamic` | 1.00 | x | x | x | x | x | x | x | x | x | x | x | x | x | x | x | x |
| `soviet-school` | 1.00 | x | x | x | x | x | x | x | x | x | x | . | x | x | x | x | x |
| `king-safety` | 1.00 | x | x | x | x | x | x | x | x | x | x | x | x | x | x | x | x |
| `luft` | 1.00 | x | x | x | . | x | x | x | x | . | x | . | . | x | x | x | x |
| `open-file` | 1.00 | x | x | x | x | x | x | x | x | x | x | . | x | x | x | x | x |
| `semi-open-file` | 1.00 | x | x | x | x | x | x | x | x | x | x | . | x | x | x | x | x |
| `middlegame-principles` | 1.00 | x | x | x | x | x | x | x | x | x | x | . | x | x | x | x | x |
| `back-rank-mate` | 1.00 | x | x | x | . | x | x | x | x | x | x | . | x | x | x | x | x |
| `building-a-bridge` | 1.00 | x | x | x | x | x | . | x | x | . | x | x | . | x | x | x | x |
| `compensation` | 1.00 | x | x | x | . | x | x | x | x | x | x | . | x | x | x | x | x |
| `early-queen-development` | 1.00 | x | x | x | . | x | x | x | x | x | x | . | x | x | x | x | x |
| `gambit` | 1.00 | x | x | x | x | x | x | . | . | . | . | . | . | x | x | x | x |
| `tabiya` | 1.00 | x | x | x | x | x | x | . | . | . | . | . | . | x | x | x | x |
| `theoretical-novelty` | 1.00 | x | x | x | . | x | x | . | . | . | . | . | . | x | x | x | x |
| `transposition` | 1.00 | x | x | x | . | x | x | x | x | . | x | . | . | x | x | x | x |
| `backward-pawn` | 1.00 | x | x | x | x | x | x | x | x | x | x | . | x | x | x | x | x |
| `doubled-pawns` | 1.00 | x | x | x | x | x | x | x | x | x | x | . | x | x | x | x | x |
| `hanging-pawns` | 1.00 | x | x | x | . | x | x | x | x | x | x | x | x | x | x | x | x |
| `isolated-queen-pawn` | 1.00 | x | x | x | x | x | x | x | x | x | x | . | x | x | x | x | x |
| `minority-attack` | 1.00 | x | x | x | x | x | x | x | x | x | x | . | x | x | x | x | x |
| `outpost` | 1.00 | x | x | x | x | x | x | x | x | x | x | . | x | x | x | x | x |
| `pawn-break` | 1.00 | x | x | x | . | x | x | x | x | x | x | x | x | x | x | x | x |
| `pawn-breakthrough` | 1.00 | x | x | x | . | x | x | x | x | x | x | x | x | x | x | x | x |
| `weak-square` | 1.00 | x | x | x | x | x | x | x | x | x | x | . | x | x | x | x | x |
| `bad-bishop` | 1.00 | x | x | x | x | x | x | x | x | x | x | . | x | x | x | x | x |
| `bishop-pair` | 1.00 | x | x | x | x | x | x | x | x | x | x | . | x | x | x | x | x |
| `knight-vs-bishop` | 1.00 | x | x | x | x | x | x | x | x | x | x | . | x | x | x | x | x |
| `piece-coordination` | 1.00 | x | x | x | x | x | x | x | x | x | x | . | x | x | x | x | x |
| `exchange-sacrifice` | 1.00 | x | x | x | x | x | x | x | x | x | x | . | x | x | x | x | x |
| `material-imbalance` | 1.00 | x | x | x | x | x | x | x | x | x | x | . | x | x | x | x | x |
| `piece-exchanges` | 1.00 | x | x | x | x | x | x | x | x | x | x | x | x | x | x | x | x |
| `positional-evaluation` | 1.00 | x | x | x | x | x | x | x | x | x | x | . | x | x | x | x | x |
| `positional-sacrifice` | 1.00 | x | x | x | x | x | x | x | x | x | x | . | x | x | x | x | x |
| `sacrifice` | 1.00 | x | x | x | x | x | x | x | x | . | x | . | . | x | x | x | x |
| `static-vs-dynamic` | 1.00 | x | x | x | x | x | x | x | x | x | x | . | x | x | x | x | x |
| `transformation-of-advantages` | 1.00 | x | x | x | x | x | x | x | x | x | x | . | x | x | x | x | x |
| `two-weaknesses` | 1.00 | x | x | x | x | x | x | x | x | x | x | x | x | x | x | x | x |
| `worst-placed-piece` | 1.00 | x | x | x | x | x | x | x | x | x | x | . | x | x | x | x | x |
| `castling` | 1.00 | x | x | x | . | x | . | x | x | . | . | . | . | x | x | x | x |
| `check` | 1.00 | x | x | x | . | x | . | x | x | . | . | x | . | x | x | x | x |
| `dead-position` | 1.00 | x | x | x | . | x | . | x | x | . | . | x | . | x | x | x | x |
| `en-passant` | 1.00 | x | x | x | . | x | . | x | x | . | . | x | . | x | x | x | x |
| `fifty-move-rule` | 1.00 | x | x | x | . | x | . | x | x | . | . | x | . | x | x | x | x |
| `notation` | 1.00 | x | x | x | x | x | . | x | x | . | . | . | . | x | x | x | x |
| `seventy-five-move-rule` | 1.00 | x | x | x | . | x | . | x | x | . | . | . | . | x | x | x | x |
| `battery` | 1.00 | x | x | x | . | x | x | x | x | x | x | . | x | x | x | x | x |
| `discovered-attack` | 1.00 | x | x | x | . | x | x | x | x | x | x | . | x | x | x | x | x |
| `discovered-check` | 1.00 | x | x | x | . | x | x | x | x | x | x | . | x | x | x | x | x |
| `double-check` | 1.00 | x | x | x | . | x | x | x | x | x | x | x | x | x | x | x | x |
| `forcing-moves` | 1.00 | x | x | x | x | x | x | x | x | x | x | x | x | x | x | x | x |
| `fork` | 1.00 | x | x | x | . | x | x | x | x | x | x | x | x | x | x | x | x |
| `hanging-piece` | 1.00 | x | x | x | . | x | x | x | x | . | x | x | . | x | x | x | x |
| `loose-piece` | 1.00 | x | x | x | . | x | x | x | x | . | x | . | . | x | x | x | x |
| `pin` | 1.00 | x | x | x | . | x | x | x | x | x | x | . | x | x | x | x | x |
| `skewer` | 1.00 | x | x | x | . | x | x | x | x | x | x | x | x | x | x | x | x |
| `tactical-combinations` | 1.00 | x | x | x | x | x | x | . | . | . | . | . | . | x | x | x | x |
| `trapped-piece` | 1.00 | x | x | x | . | x | x | x | x | x | x | . | x | x | x | x | x |
| `fortress` | 0.94 | x | x | x | x | x | x | x | x | x | x | x | - | x | x | x | x |
| `knight-endgames` | 0.94 | x | x | x | x | x | x | x | x | x | x | x | - | x | x | x | x |
| `mixed-piece-endgames` | 0.94 | x | x | x | x | x | x | x | x | x | x | x | - | x | x | x | x |
| `queen-endgames` | 0.94 | x | x | x | x | x | x | x | x | x | x | x | - | x | x | x | x |
| `rook-endgame-principles` | 0.94 | x | x | x | x | x | x | x | x | x | x | x | - | x | x | x | x |
| `lucena-position` | 0.94 | x | x | x | x | x | x | x | x | x | x | x | - | x | x | x | x |
| `philidor-position` | 0.94 | x | x | x | x | x | x | x | x | x | x | x | - | x | x | x | x |
| `vancura-position` | 0.94 | x | x | x | x | x | x | x | x | x | x | x | - | x | x | x | x |
| `passed-pawn` | 0.94 | x | x | x | x | x | x | x | x | x | x | - | x | x | x | x | x |
| `piece-activity` | 0.94 | x | x | x | x | x | x | x | x | x | x | - | x | x | x | x | x |
| `rook-on-the-seventh` | 0.94 | x | x | x | x | x | x | x | x | x | x | - | x | x | x | x | x |
| `king-attack` | 0.93 | x | x | x | x | x | x | - | x | x | x | . | x | x | x | x | x |
| `lpdo` | 0.93 | x | x | x | x | x | x | x | x | x | x | . | - | x | x | x | x |
| `smothered-mate` | 0.93 | x | x | x | . | x | x | x | x | x | x | x | - | x | x | x | x |
| `hole` | 0.93 | x | x | x | x | x | x | x | x | x | x | . | - | x | x | x | x |
| `strong-square` | 0.93 | x | x | x | x | x | x | x | x | x | x | . | - | x | x | x | x |
| `double-attack` | 0.93 | x | x | x | . | x | x | x | x | x | x | x | - | x | x | x | x |
| `greek-gift` | 0.93 | x | x | x | x | x | x | x | x | x | x | . | - | x | x | x | x |
| `knight-fork` | 0.93 | x | x | x | . | x | x | x | x | x | x | x | - | x | x | x | x |
| `x-ray` | 0.93 | x | x | x | . | x | x | x | x | x | x | x | - | x | x | x | x |
| `pawn-storm` | 0.93 | x | x | x | . | x | x | - | x | x | x | . | x | x | x | x | x |
| `queen-activity` | 0.93 | x | x | x | . | x | x | - | x | x | x | . | x | x | x | x | x |
| `deflection` | 0.93 | x | x | x | . | x | x | x | x | - | x | . | x | x | x | x | x |
| `overloading` | 0.93 | x | x | x | . | x | x | x | x | x | x | . | - | x | x | x | x |
| `removing-the-defender` | 0.93 | x | x | x | . | x | x | x | x | - | x | . | x | x | x | x | x |
| `zwischenzug` | 0.93 | x | x | x | . | x | x | x | x | x | x | . | - | x | x | x | x |
| `triangulation` | 0.92 | x | x | x | x | x | . | - | x | . | x | x | . | x | x | x | x |
| `insufficient-material` | 0.92 | x | x | x | . | x | x | x | x | . | - | x | . | x | x | x | x |
| `checkmate` | 0.91 | x | x | x | . | x | . | x | x | . | . | x | . | - | x | x | x |
| `stalemate` | 0.91 | x | x | x | . | x | . | x | x | . | . | x | . | - | x | x | x |
| `piece-properties` | 0.88 | x | x | x | x | x | x | - | x | x | x | x | - | x | x | x | x |
| `decoy` | 0.86 | - | x | x | . | x | x | x | x | - | x | . | x | x | x | x | x |
| `desperado` | 0.86 | - | x | x | . | x | x | x | x | x | x | . | - | x | x | x | x |
| `exceptions-to-rules` | 0.85 | x | x | x | . | x | x | - | x | . | - | x | . | x | x | x | x |
| `candidate-moves` | 0.80 | x | x | x | x | x | x | x | - | - | x | . | x | - | x | x | x |
| `promotion` | 0.80 | x | x | x | . | x | . | x | - | . | . | . | . | - | x | x | x |
| `threefold-repetition` | 0.80 | x | x | x | . | x | . | x | - | . | . | . | . | - | x | x | x |
| `corresponding-squares` | 0.77 | x | x | x | x | x | . | - | x | . | - | - | . | x | x | x | x |
| `planning` | 0.73 | x | x | x | x | x | x | - | x | - | - | . | - | x | x | x | x |
| `clearance` | 0.71 | x | x | x | . | x | x | - | x | - | - | . | - | x | x | x | x |
| `interference` | 0.71 | x | x | x | . | x | x | - | x | - | - | . | - | x | x | x | x |
| `fivefold-repetition` | 0.70 | - | x | x | . | x | . | x | - | . | . | . | . | - | x | x | x |
| `blindfold-interference` | 0.67 | x | x | x | x | x | . | - | - | . | - | . | . | - | x | x | x |
| `blindfold-representation` | 0.67 | x | x | x | x | x | . | - | - | . | - | . | . | - | x | x | x |
| `chunking` | 0.67 | x | x | x | x | x | . | - | - | . | - | . | . | - | x | x | x |
| `beginner-misconceptions` | 0.67 | x | x | x | . | x | x | - | - | . | - | . | . | - | x | x | x |
| `named-strategic-concepts` | 0.67 | x | x | x | . | x | x | - | - | . | - | . | . | - | x | x | x |
| `chess-terminology` | 0.67 | x | x | x | . | x | x | - | - | . | - | . | . | - | x | x | x |
| `board-vision` | 0.60 | x | x | x | x | x | x | - | - | - | - | . | - | - | x | x | x |
| `calculation` | 0.60 | x | x | x | x | x | x | - | - | - | - | . | - | - | x | x | x |
| `practical-principles` | 0.60 | x | x | x | x | x | x | - | - | - | - | . | - | - | x | x | x |
| `practical-psychology` | 0.60 | x | x | x | x | x | x | - | - | - | - | . | - | - | x | x | x |
| `time-management` | 0.60 | x | x | x | x | x | x | - | - | - | - | . | - | - | x | x | x |
| `advanced-concepts` | 0.58 | x | x | x | . | x | - | - | - | . | - | . | . | - | x | x | x |
| `intermediate-concepts` | 0.58 | x | x | x | . | x | - | - | - | . | - | . | . | - | x | x | x |
| `master-concepts` | 0.58 | x | x | x | . | x | - | - | - | . | - | . | . | - | x | x | x |
| `opening-repertoire` | 0.57 | x | x | x | . | x | x | - | - | - | - | . | - | - | x | x | x |
