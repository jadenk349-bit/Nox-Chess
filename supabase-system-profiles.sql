...
--
--     No ranked game between people can involve one (nothing can sign in as
--     one). The one thing that rates a game these rows play is
--     record_rated_game() in supabase-migrate-visions.sql, which switches the
--     flag below on for its own transaction, exactly as this file does for
--     its own writes. This trigger is for the other ways a rating changes: a bulk `update profiles set rating = …`
